...
from ttmake.private.utility import norm_path, get_path, format_message, glob_recursive, open_yaml
...
def check_module_size(paths):
    ...
    try:
        ...
        error_message = messages.error_message % 'check_module_size'
...
def get_modified_sources(paths,
    ...
    ``source_map`` (returned by :ref:`moving source/output functions<moving source/output functions>`).
    ...
    paths : dict
        Dictionary of paths. Dictionary should contain values for all keys listed below.
    source_map : list
        Mapping of sources (returned from :ref:`moving source/output functions<moving source/output functions>`).
    ...
    try:
        ...
        error_message = messages.error_message % 'get_modified_sources'
        error_message = format_message(error_message)
        write_to_makelog(paths, error_message + '\n\n' + traceback.format_exc())
        raise_from(ColoredError(error_message, traceback.format_exc()), None)
